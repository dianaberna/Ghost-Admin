...
export default class EventTimeline extends Component {
    ...
    getAction(event) {
        ...
        if (event.type === 'payment_event') {
            return this.intl.t('timeline.made a payment');
        }

        if (event.type === 'newsletter_event') {
            if (event.data.subscribed) {
                return this.intl.t('timeline.subscribed to');
            } else {
                return this.intl.t('timeline.unsubscribed from');
            }
        }

        if (event.type === 'subscription_event') {
            if (event.data.from_plan === null) {
                return this.intl.t('timeline.started');
            }

            if (event.data.to_plan === null) {
                return this.intl.t('timeline.cancelled');
            }

            return this.intl.t('timeline.changed');
        }
    }
