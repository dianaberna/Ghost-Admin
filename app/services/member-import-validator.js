...
export default Service.extend({
    ...
    async check(data) {
        if (!data || !data.length) {
            return {
                validationErrors: [new MemberImportError({
                    message: this.intl.t('members.File is empty, nothing to import. Please select a different file.')
                })]
            };
        }
        ...
        if (!hasEmails) {
            validationErrors.push(new MemberImportError({
                message: this.intl.t('members.No email addresses found in the uploaded CSV.')
            }));
        } else {
            // check can be done on whole set as it won't be too slow
