...
export function formatPostTime(timeago, {timezone = 'ect/UTC', draft, scheduled, published}) {
    ...
    if (time.isSame(now, 'day')) {
        return scheduled
            ? time.format(this.intl.t(`moment.[at] HH:mm [${utcOffset}] [Today]`))
            : time.format(this.intl.t(`moment.HH:mm [${utcOffset}] [Today]`));
    }
    ...
    if (published && time.isSame(now.clone().subtract(1, 'days').startOf('day'), 'day')) {
        return time.format(this.intl.t(`moment.HH:mm [${utcOffset}] [Yesterday]`));
    }

    // if scheduled for tomorrow, render the time + Tomorrow
    if (scheduled && time.isSame(now.clone().add(1, 'days').startOf('day'), 'day')) {
        return time.format(this.intl.t(`moment.[at] HH:mm [${utcOffset}] [Tomorrow]`));
    }

    // Else, render just the date if published, or the time & date if scheduled
    let format = scheduled
        ? this.intl.t(`moment.[at] HH:mm [${utcOffset}] [on] DD MMM YYYY`)
        : this.intl.t('moment.DD MMM YYYY');
...
export default class GhFormatPostTimeHelper extends Helper {
    ...
    compute([timeago], options) {
        ...
        return (formatPostTime.bind(this))(timeago, Object.assign({}, options, {timezone: this.settings.get('timezone')}));
    }
}
